...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    (for case (block, move) <- b.legalNeighbors yield (block, move :: history)).to(LazyList)
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    case (block, moves) #:: tail =>
      ...
      val newNewighbors = newNeighborsOnly(neighbors, explored)
      (block, moves) #:: from(tail ++ newNewighbors, explored + block)
